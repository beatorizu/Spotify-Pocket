...
import { removeTrackFromPlayer, setPlayerHeight } from '../../actions/content';
...
const Player = () => {
  ...
  const previousId = usePrevious(playingNowId);
  ...
  useEffect(() => {
    if (playingNowId === previousId) {
      return;
    }
    setCurrentTrack(playingNowTrack)
  }, [playingNowId, playingNowTrack, previousId])

  useEffect(() => {
    const audioPlayer = audioElementRef.current;

    if (previousId === playingNowId) {
      if (isPlaying && audioPlayer?.paused) {
        audioPlayer.play();
      }
      if (!isPlaying && !audioPlayer?.paused) {
  ...
  }, [isPlaying, playingNowId, previousId]);
